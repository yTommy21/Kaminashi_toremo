...
public class CameraFollow : MonoBehaviour 
{
	...
	[System.Serializable]
	public class Param {
		public CAMERATARGET tragetType 			= CAMERATARGET.PLAYER_MARGIN;
		public CAMERAHOMING homingType 			= CAMERAHOMING.DIRECT;
		public Vector2 		margin 				= new Vector2 (-0.04f, 2.48f);
	...
	void Awake () {
		...
		newPlayerCtrl  = player.GetComponent<NewPlayerController>();
	}
	...
	void LateUpdate () {
		...
		case CAMERATARGET.PLAYER_MARGIN	:
			targetX = playerTrfm.position.x + param.margin.x * newPlayerCtrl.dir;
			targetY = playerTrfm.position.y + param.margin.y;
			break;
		case CAMERATARGET.PLAYER_GROUND	:
			targetX = playerTrfm.position.x + param.margin.x * newPlayerCtrl.dir;
			targetY = newPlayerCtrl.groundY + param.margin.y;
			break;
		}
		...
		if (param.viewAreaCheck) {
			...
			if (playerTrfm.position.y < min.y || playerTrfm.position.y > max.y) {
				targetY = playerTrfm.position.y;
				newPlayerCtrl.groundY = playerTrfm.position.y;
			}
		}
